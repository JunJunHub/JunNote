...
```c
//C、C++业务接口注释模板, 参照科达注释规范

// ============================================================
// 函数名: funcName
// 功  能: 监测数据库磁盘使用情况线程
//         磁盘使用率超过 90% ,删除数据库 binlog
//         防止某些操作致使 binglog 增长过快,撑爆磁盘
//
// 参  数: u32 dwXXXId [in]XXXID
//         u8 *pbyS    [out]返回
//
// 返回值: BOOL
//         true        成功
...
// 修改人:
// 日  期        版  本        修改人        修改内容
// 2024/02/28     1.0          李永军        创建
// 2024/06/01     1.1          李永军        增加X的判断逻辑
//                                           描述太长得换行
// 2021/07/05     1.2          李永军        XXXXXXXX      
// ============================================================
...
// @Produce json
// @Success 200 {object} response.Response{data=[]string} "告警提示音列表"
// @Router 	/monitor/alarm/sound/alarmToneList [GET]


//Server等其它业务代码接口, 参照科达注释规范
...
## 通道轮询成员表错误--某现场BUG
-- 查看冲突的轮询配置数据  
SELECT loop_cfg_id FROM mpu_tv_wall_loop_member_backup WHERE loop_cfg_id NOT IN (SELECT loop_cfg_id FROM mpu_tv_wall_loop_cfg_backup);  

--  删除冲突的数据 (先确认数据是否有效)
DELETE FROM mpu_tv_wall_loop_member_backup WHERE loop_cfg_id NOT IN (SELECT loop_cfg_id FROM mpu_tv_wall_loop_cfg_backup);
...
touch /tmp/ipc_debug  开启IPC通讯相关打印
touch /tmp/preview_debug 保存接收到的预览图像

开启 eglfs 插件调试日志(因 log4qt 已接管 qdebug 日志 log4qt 日志级别也需设置为 debug)
...
1、web上去除了日志删除按钮  
2、nginx 相关漏洞修复
3、mpuaps 业务进程使用 nginx 代理对外提供服务 
    - V2R1B4版本之后，nginx 代理 mpuaps 监听端口默认是80 443; mpuaps进程自身监听端口是9180,9181;
...
// 同时打开多个模块调试打印
// 注：log4qt接管了Qt的日志输出，注意设置log4qt日志级别
